...
### Задание
На предыдущем уроке вы создали REST API для модуля "Рабочее место администратора системы". Адаптируйте код, написанный в рамках урока 8.
Напомню, что мы использовали массивы во vuex для временного хранения данных. Теперь, применив библиотеку [Axios](https://github.com/axios/axios) для доступа к API,
отправляйте данные на сервер. 
### Задание для самоконтроля
1. Чем результат выполнения асинхронной задачи отличается от результата выполнения синхронной задачи?
1. Убедитесь, что данные сохраняются на сервере и не теряются после закрытия браузера.
